...
namespace NPCBrain.Archetypes
{
    ...
    public class HearingGuardNPC : NPCBrainController, IAlertableNPC
    {
        ...
        public SoundType? LastSoundType => Blackboard.Has(BBKeys.LastSoundType) 
            ? (SoundType?)Blackboard.GetInt(BBKeys.LastSoundType, 0) 
            : null;
        ...
        private void HandleSoundHeard(SoundEvent sound)
        {
            ...
            if (!Blackboard.Has(BBKeys.Target))
            {
                ...
                if (Blackboard.Has(BBKeys.LastSoundType))
                {
                    var currentType = (SoundType)Blackboard.GetInt(BBKeys.LastSoundType, 0);
        ...
        private UtilityAction CreateGunshotInvestigateAction()
        {
            var investigateBehavior = new Sequence(
                ...
                new SetBlackboard(BBKeys.CurrentState, "Investigate-Gunshot"),
                new MoveTo(
            ...
            return new UtilityAction(
                ...
                // Higher score when alert level is high
                new BlackboardConsideration<float>("AlertForGunshot", BBKeys.AlertLevel,
                    a => 0.3f + a * 0.7f, 0f),
        ...
        private UtilityAction CreateFootstepInvestigateAction()
        {
            var investigateBehavior = new Sequence(
                new SetBlackboard(BBKeys.LastInvestigateTime, () => Time.time),
                new SetBlackboard(BBKeys.CurrentState, "Investigate-Footstep"),
                new MoveTo(
                    () => Blackboard.GetVector3(BBKeys.InvestigatePosition, Vector3.zero),
                    _arrivalDistance,
                    _investigateSpeed
            ...
            return new UtilityAction(
                ...
                new BlackboardConsideration<float>("AlertForFootstep", BBKeys.AlertLevel,
